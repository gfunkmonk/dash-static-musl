...
autoreconf -f -i && \
./configure --with-libedit --enable-static LDFLAGS='-static -ledit -lncursesw -ltinfo -Wl,--gc-sections' PKG_CONFIG='pkg-config --static' CFLAGS='-Os -ffunction-sections -fdata-sections -Wno-misleading-indentation -Wno-maybe-uninitialized' && \
make -j\$(nproc) && \
strip src/dash && \
upx --lzma src/dash"
mkdir -p dist
