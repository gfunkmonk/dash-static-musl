...
autoreconf -f -i && \
./configure --with-libedit --enable-static LDFLAGS='-static -L/usr/local/lib -lncurses -ltinfo -ledit' CFLAGS='-Os -I/usr/local/include -no-pie -fomit-frame-pointer -fstack-clash-protection' && \
make -j\$(nproc) && \
strip src/dash && \
upx --lzma src/dash"
mkdir -p dist
